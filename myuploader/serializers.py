...
class CompositeSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        if self.instance:  # 更新の場合は, 自分が同名ファイルとして出てくるので除外
        ...
        if 'src' in attrs:
            src = attrs['src']
            # 送られたファイルの中身があるが, ディレクトリ指定
            if is_dir and src:
                raise serializers.ValidationError('ディレクトリの時は, ファイルの添付不可')

            # ファイルフラグだが, ファイルの中身が空
            if not is_dir and not src:
                raise serializers.ValidationError('ファイルの時は, ファイルの添付必須')
        ...
        # ファイルの送信がなかった
            ...
            if not self.instance:
                ...
                # ファイルの送信なく, アップロード済みでもないがファイルフラグ
                if not is_dir and not src:
                    raise serializers.ValidationError('ファイルの時は, ファイルの添付必須')
        return attrs
